...
- **Dev-Time Type Validation**: In development mode (`window.lnDebug` active or `[data-ln-debug]` present), `validateAttrValue` checks all attribute mutations and initial DOM values against the declared `type` and `values` contract, emitting descriptive console warnings with component tags and expected constraints (bypassed in production).
