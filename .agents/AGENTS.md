...
- **Typed Attribute Contracts & Closed Taxonomy**: Every component MUST explicitly declare its attribute contract via an `ATTRIBUTES` map passed to `registerComponent` and `attrSpec()`. Each entry defines `type` (from closed taxonomy: `'enum'`, `'integer'`, `'float'`, `'boolean'`, `'string'`, `'list'`, `'json'`, `'trigger'`, `'marker'`), `values` (for enums), `fallback`, `min`/`max` (for numbers), and `description`.
- **Auto-Inferred Readers**: When `read` is omitted in an attribute spec, `attrSpec()` automatically infers the reader parser from `type` (`attrInt`, `attrFloat`, `attrBool`, `attrEnum(entry.values, entry.fallback)`, `attrJson`, `attrStr`).
- **Dev-Time Type Validation**: In development mode (`window.lnDebug` active or `[data-ln-debug]` present), `validateAttrValue` checks all attribute mutations and initial DOM values against the declared `type` and `values` contract, emitting descriptive console warnings with component tags and expected constraints (`0ns` runtime overhead in production).
- **Automated Schema Synchronization**: Whenever component attributes are added or modified, run `npm run sync:ln-schemas` to update `.schema.json` files and the centralized attribute catalog.
