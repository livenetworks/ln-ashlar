...
function oe(d, a, y) {
  ...
  b[y] = d.name, S(d.dom, a, b);
}
function it(d, a) {
  if (!d || !a) return d;
  const y = d.querySelectorAll("[data-ln-field]");
  for (let h = 0; h < y.length; h++) {
    const s = y[h], m = s.getAttribute("data-ln-field");
    a[m] != null && (s.textContent = a[m]);
  }
  const b = d.querySelectorAll("[data-ln-attr]");
  for (let h = 0; h < b.length; h++) {
    const s = b[h], m = s.getAttribute("data-ln-attr").split(",");
    for (let o = 0; o < m.length; o++) {
      const l = m[o].trim().split(":");
      if (l.length !== 2) continue;
      const u = l[0].trim(), c = l[1].trim();
      a[c] != null && s.setAttribute(u, a[c]);
    }
  }
  const _ = d.querySelectorAll("[data-ln-show]");
  for (let h = 0; h < _.length; h++) {
    const s = _[h], m = s.getAttribute("data-ln-show");
    m in a && s.classList.toggle("hidden", !a[m]);
  }
  const p = d.querySelectorAll("[data-ln-class]");
  for (let h = 0; h < p.length; h++) {
    const s = p[h], m = s.getAttribute("data-ln-class").split(",");
    for (let o = 0; o < m.length; o++) {
      const l = m[o].trim().split(":");
      if (l.length !== 2) continue;
      const u = l[0].trim(), c = l[1].trim();
      c in a && s.classList.toggle(u, !!a[c]);
    }
  }
  return d;
}
function Le(d, a) {
  ...
  for (let b = 0; b < y.length; b++)
    y[b].dispatchEvent(new CustomEvent("ln-fill", { detail: a ?? null, bubbles: !0 }));
  return d;
}
...
function ft(d, a, y) {
  if (d) {
    const b = d.querySelector('[data-ln-template="' + a + '"]');
    if (b) return b.content.cloneNode(!0);
  }
  return kt(a, y);
}
function Wt(d, a) {
  const y = {}, b = d.querySelectorAll("[" + a + "]");
  for (let _ = 0; _ < b.length; _++)
    y[b[_].getAttribute(a)] = b[_].textContent, b[_].remove();
  return y;
}
function Bt(d, a, y, b) {
  if (d.nodeType !== 1) return;
  const p = a.indexOf("[") !== -1 || a.indexOf(".") !== -1 || a.indexOf("#") !== -1 ? a : "[" + a + "]", h = Array.from(d.querySelectorAll(p));
...
function se(d, a) {
  const y = !!(a && a.typed), b = a && a.exclude, _ = {}, p = d.elements, h = {};
  if (y)
...
function Dt(d) {
  ...
  for (let y = 0; y < d.length; y++) {
    const b = d[y];
    if (!(b === "" || b == null) && (a = !0, !Number.isFinite(Number(b))))
      return "string";
  }
...
function H(d, a, y, b, _ = {}) {
  ...
  return ct(function() {
    const o = new MutationObserver(function(u) {
      for (let c = 0; c < u.length; c++) {
        ...
        } else f.type === "attributes" && (h && f.target[a] ? h(f.target, f.attributeName) : (Bt(f.target, d, a, y), s && s(f.target)));
      }
    });
    let l = [];
    if (d.indexOf("[") !== -1) {
  ...
  }, b || (d.indexOf("[") === -1 ? d.replace("data-", "") : "component")), window[a] = m, document.readyState === "loading" ? document.addEventListener("DOMContentLoaded", function() {
    m(document.body);
  }) : m(document.body), m;
}
function ce(d, a) {
...
function De(d) {
  return ue[d] || { ingress: (a) => a, egress: (a) => a };
}
const he = {};
function Gt(d, a) {
...
function fe(d) {
  d = d || {};
  let a = d.windowSize > 0 ? d.windowSize : 1e3, y = d.pageSize > 0 ? d.pageSize : 200, b = d.threshold != null ? d.threshold : 25, _ = d.fetchDebounce != null ? d.fetchDebounce : 120;
  const p = typeof d.requestPage == "function" ? d.requestPage : function() {
  }, h = typeof d.onChange == "function" ? d.onChange : function() {
  }, s = /* @__PURE__ */ new Map(), m = /* @__PURE__ */ new Map(), o = /* @__PURE__ */ new Set();
  let l = 0, u = 0, c = 0, f = { sort: null, filters: {}, search: "" }, e = null, r = 0, t = 0, n = !1;
  function i(E) {
    m.set(E, ++r);
  }
  function g() {
  ...
  function v() {
    if (s.size <= a) return;
    const E = Array.from(s.keys()).sort(function(L, q) {
      return (m.get(L) || 0) - (m.get(q) || 0);
    });
    let A = 0;
    for (; s.size > a && A < E.length; )
      s.delete(E[A]), m.delete(E[A]), A++;
  }
  function w(E, A) {
    o.add(E), p(f, E, A);
  }
  return {
    ...
    ingest: function(E) {
      ...
      n && (s.clear(), m.clear(), n = !1), u = E.total != null ? E.total : u, l = E.filtered != null ? E.filtered : E.data ? E.data.length : l;
    ...
    invalidate: function(E) {
      c++, o.clear(), clearTimeout(e), E && (f = E), n = !0, w(0, y);
    },
    ...
    destroy: function() {
      clearTimeout(e), s.clear(), m.clear(), o.clear();
    },
    configure: function(E) {
      ...
      E.pageSize != null && E.pageSize > 0 && (y = E.pageSize), E.threshold != null && E.threshold >= 0 && (b = E.threshold), E.fetchDebounce != null && E.fetchDebounce >= 0 && (_ = E.fetchDebounce), A && h();
    },
    setGrandTotal: function(E) {
      E == null || isNaN(E) || E < 0 || (u = E, g() || (l = E), h());
    }
  };
}
const Ie = "ln:";
...
function me(d, a) {
  const y = a.getAttribute("data-ln-persist"), b = y !== null && y !== "" ? y : a.id;
  return b ? Ie + d + ":" + Re() + ":" + b : (console.warn('[ln-persist] Element requires id or data-ln-persist="key"', a), null);
}
function Mt(d, a) {
  if (!pe()) return null;
  const y = me(d, a);
...
function Nt(d) {
  const a = d === void 0 ? location.hash : d, y = {}, b = _e(a);
  if (!b) return y;
  const _ = b.split("&");
  for (let p = 0; p < _.length; p++) {
    const h = _[p];
    if (!h) continue;
    const s = h.indexOf(":"), m = s > -1 ? h.slice(0, s) : h, o = s > -1 ? h.slice(s + 1) : "";
    if (m)
      try {
...
function Rt(d, a, y, b) {
  const _ = typeof b == "number" ? b : 4, p = window.innerWidth, h = window.innerHeight, s = a.width, m = a.height, o = (y || "bottom").split("-"), l = o[0], u = o[1] === "start" || o[1] === "end" ? o[1] : "center", c = {
  ...
  function e(g) {
    return g === "top" || g === "bottom" ? u === "start" ? d.left : u === "end" ? d.right - s : d.left + (d.width - s) / 2 : u === "start" ? d.top : u === "end" ? d.bottom - m : d.top + (d.height - m) / 2;
  }
  function r(g) {
    let v, w, E = !0;
    return g === "top" ? (v = d.top - _ - m, w = e(g), v < 0 && (E = !1)) : g === "bottom" ? (v = d.bottom + _, w = e(g), v + m > h && (E = !1)) : g === "left" ? (v = e(g), w = d.left - _ - s, w < 0 && (E = !1)) : (v = e(g), w = d.right + _, w + s > p && (E = !1)), { top: v, left: w, side: g, fits: E };
  }
  let t = null;
  ...
  return s >= p ? i = 0 : (i < 0 && (i = 0), i + s > p && (i = p - s)), m >= h ? n = 0 : (n < 0 && (n = 0), n + m > h && (n = h - m)), { top: n, left: i, placement: t.side };
}
function Ut(d) {
  if (!d) return { width: 0, height: 0 };
  const a = d.style, y = a.visibility, b = a.display, _ = a.position;
  a.visibility = "hidden", a.display = "block", a.position = "fixed";
  const p = d.offsetWidth, h = d.offsetHeight;
  return a.visibility = y, a.display = b, a.position = _, { width: p, height: h };
}
let ut = null;
async function Zt(d) {
  if (!d) {
  ...
  try {
    ...
    console.error("[ln-core/crypto] Key derivation failed:", a), ut = null;
  }
}
function gt() {
  return ut;
}
async function Oe(d, a = ut) {
  const y = a || ut;
  ...
  try {
    const b = new TextEncoder(), _ = crypto.getRandomValues(new Uint8Array(12)), p = typeof d == "string" ? d : JSON.stringify(d), h = await crypto.subtle.encrypt(
      ...
      y,
      b.encode(p)
    ), s = btoa(String.fromCharCode(..._)), m = btoa(String.fromCharCode(...new Uint8Array(h)));
    return {
      encrypted: !0,
      iv: s,
      data: m
...
(function() {
  ...
  function s(o, l) {
    ...
    h(c) && a.has(f) && (a.get(f).abort(), a.delete(f));
    ...
    return a.set(f, e), d(o, n).finally(function() {
      r && t && r.removeEventListener("abort", t), a.get(f) === e && a.delete(f);
    });
  }
  s.toString = function() {
  ...
  document.addEventListener("ln-http:request", m), window.lnHttp = {
    cancel: function(o) {
      ...
      return a.forEach(function(u, c) {
        c.endsWith(" " + o) && (u.abort(), a.delete(c), l = !0);
      }), l;
    },
...
(function() {
  const d = "data-ln-form", a = "lnForm", y = "data-ln-form-action-edit", b = "data-ln-form-action-method";
  ...
  _.prototype.fill = function(p) {
    ...
    for (let s = 0; s < h.length; s++) {
      const m = h[s], o = m.tagName === "SELECT" || m.type === "checkbox" || m.type === "radio";
    ...
    if (h !== null) {
      ...
      m ? this.dom.setAttribute("action", m.replace(":id", encodeURIComponent(h))) : this.dom.setAttribute("action", this._baseAction.replace(/\/$/, "") + "/" + encodeURIComponent(h)), s.value = this.dom.getAttribute(b) || "PUT";
    } else
...
(function() {
  const d = "data-ln-validate", a = "lnValidate", y = "data-ln-validate-errors", b = "data-ln-validate-error", _ = "ln-validate-valid", p = "ln-validate-invalid", h = {
  ...
  s.prototype.validate = function() {
    ...
    m && (this._onFormReset && m.removeEventListener("reset", this._onFormReset), this._onValidateRequest && m.removeEventListener("ln-validate:request-validate", this._onValidateRequest)), this.dom.classList.remove(_, p), this.dom.removeAttribute("aria-invalid"), S(this.dom, "ln-validate:destroyed", { target: this.dom }), delete this.dom[a];
  }, H(d, a, s, "ln-validate");
})();
(function() {
  ...
  if (window[a] !== void 0) return;
  function b(u) {
    if (!u.hasAttribute(d) || u[a]) return;
    u[a] = !0;
    const c = m(u);
    _(c.links), p(c.forms);
  }
  function _(u) {
    for (const c of u) {
      ...
      c.addEventListener("click", e), c[a + "Trigger"] = e;
    }
  }
  function p(u) {
    for (const c of u) {
  ...
  function s(u, c, f, e, r) {
    ...
    fetch(g, E).then(function(A) {
      ...
      if (A.ok) {
        ...
        S(e, "ln-ajax:error", { method: u, url: g, status: A.status, data: L });
      if (L.message) {
        const q = L.message;
      ...
      S(e, "ln-ajax:error", { method: u, url: g, error: A }), S(e, "ln-ajax:complete", { method: u, url: g }), i();
    });
  }
  function m(u) {
    const c = { links: [], forms: [] };
  ...
  window[a] = b, window[a].destroy = h, o(), document.readyState === "loading" ? document.addEventListener("DOMContentLoaded", l) : l();
})();
const ge = {
  navigate: function(d) {
...
function we(d, a) {
  ...
  const y = d.segments, b = a.segments, _ = Math.max(y.length, b.length);
  for (let p = 0; p < _; p++) {
    const h = y[p], s = b[p];
    ...
    if (h === "*") return 1;
    if (s === "*") return -1;
    const m = h.startsWith(":"), o = s.startsWith(":");
    if (m && !o) return 1;
    if (!m && o) return -1;
  }
  return 0;
...
function Vt(d, a) {
  if (d !== "__primary__") {
    const b = document.getElementById(a.target);
    return b || console.warn(`[ln-router] Explicit target element #${a.target} not found in DOM`), b;
  }
...
function Ct(d, a = {}) {
  ...
  let s = null;
  if (h && (s = Vt("__primary__", h.route), !s || $(s, "ln-router:before-navigate", {
    from: zt,
    to: d,
    params: h.params,
    query: b
  ...
  const o = function() {
    for (const { regionKey: l, match: u, targetEl: c } of m) {
      ...
      if (te.set(c, u.route.templateNode), l === "__primary__" && (u.route.title && (document.title = u.route.title), !a.isHydration)) {
...
function Fe(d) {
  ...
  if (!a || !ce(d, a)) return;
  const y = a.getAttribute("href"), { path: b } = Ot(y), _ = lt.get("__primary__");
  if (!_) return;
  Ae(b, _.sorted) && (d.preventDefault(), Ct(y, { historyAction: "push" }));
}
function Pe(d, a) {
  const y = Object.keys(d), b = Object.keys(a);
...
function Be() {
  const d = window.location.pathname + window.location.search, a = ge.current();
  if (a && a.path != null) {
    const y = Ot(d);
    if (Ot(a.path).path === y.path && Pe(a.query, y.query))
      return;
  }
...
function Ue(d) {
  ...
  const _ = lt.get(b);
  if (_.routes.has(a)) {
    console.warn(`[ln-router] Duplicate route pattern registered: "${a}" in region "${b}"`);
    return;
  }
  const p = d.getAttribute("data-ln-route-title"), h = a.split("/").filter(Boolean), s = {
    pattern: a,
    segments: h,
    target: y,
    title: p,
  ...
  m && m.contains(d) && console.warn(`[ln-router] Route template with pattern "${a}" is declared inside its own outlet element:`, d), _.routes.set(a, s), _.sorted = Array.from(_.routes.values()).sort(we);
}
function ze(d) {
  const a = d.getAttribute(Yt);
  if (!a) return;
  const b = d.getAttribute("data-ln-route-target") || null || "__primary__", _ = lt.get(b);
  _ && (_.routes.delete(a), _.sorted = Array.from(_.routes.values()).sort(we), _.routes.size === 0 && lt.delete(b));
}
function Se(d) {
...
(function() {
  ...
  function y(_) {
    ...
    return this._onRequestOpen = function() {
      ...
      s && p.dom.contains(s) && (h.preventDefault(), p.dom.setAttribute(d, "close"));
    ...
    if (c) {
      ...
      if (r && r.lnModal) {
        ...
        for (const w in i) {
          ...
          const E = w.slice(7);
          E && (n[E.charAt(0).toLowerCase() + E.slice(1)] = i[w]);
        }
...
(function() {
  ...
  h.prototype._initTextElement = function() {
    ...
    if (l.groupSep && (e = e.split(l.groupSep).join("")), e = e.replace(l.decimalSep, "."), u.endsWith(l.decimalSep) || u.endsWith(".")) {
      const A = e.replace(/\.$/, ""), L = parseFloat(A);
    ...
    if (t !== null && r !== -1) {
      ...
      e.slice(r + 1).length > A && (e = e.slice(0, r + 1 + A));
    }
    ...
    if (t !== null)
      ...
      const A = r !== -1 ? e.slice(r + 1).length : 0;
      if (A > 0) {
        const L = G(o) + "|u" + A;
        y[L] || (y[L] = new Intl.NumberFormat(G(o), { useGrouping: !0, minimumFractionDigits: A, maximumFractionDigits: A })), v = y[L].format(n);
      } else
        v = l.fmt.format(n);
    }
    ...
    w > 0 && (E = v.length), o.setSelectionRange(E, E), this._setHiddenRaw(n), S(o, "ln-number:input", { value: n, formatted: v });
    ...
    this.dom[a] && (this.isTextElement || (this.dom.removeEventListener("input", this._onInput), this.dom.removeEventListener("paste", this._onPaste), this._hidden && (this.dom.name = this._hidden.name, this._hidden.remove()), this.dom.type = "number", this.dom.removeAttribute("inputmode")), S(this.dom, "ln-number:destroyed", { target: this.dom }), delete this.dom[a]);
  };
  function m() {
...
(function() {
  ...
  function m(i, g, v) {
    ...
    let x, D;
    const k = St(v), M = (v || "").toLowerCase().split("-")[0], B = _(v, { month: "long" }).resolvedOptions().locale.toLowerCase().split("-")[0], z = k && B !== M;
    z && k.monthsLong ? x = k.monthsLong[E] : x = _(v, { month: "long" }).format(i), z && k.monthsShort ? D = k.monthsShort[E] : D = _(v, { month: "short" }).format(i);
    const j = {
      yyyy: String(A),
      yy: String(A).slice(-2),
      MMMM: x,
    ...
    return g.replace(/yyyy|yy|MMMM|MMM|MM|M|dd|d|HH|mm/g, function(U) {
      return j[U];
    });
  }
  function o(i, g, v) {
    const w = s(g);
    if (w) {
      const E = _(v, w), A = (v || "").toLowerCase().split("-")[0], L = E.resolvedOptions().locale.toLowerCase().split("-")[0];
      return St(v) && L !== A ? m(i, "dd.MM.yyyy", v) : E.format(i);
    }
    return m(i, g, v);
  }
  function l(i) {
    if (!i) return "";
    const g = i.getFullYear(), v = String(i.getMonth() + 1).padStart(2, "0"), w = String(i.getDate()).padStart(2, "0");
    return g + "-" + v + "-" + w;
  }
  function u(i, g, v) {
  ...
  function c(i, g, v, w) {
    i._setHiddenRaw(g), b.set.call(i._picker, g), i._lastISO = g, w !== void 0 ? (i._isFormatting = !0, i.dom.value = w, i._isFormatting = !1) : v && i._displayFormatted(v), u(i, g, v);
  }
  function f(i) {
  ...
  e.prototype._initTextElement = function() {
    const i = this.dom;
    let g = i.getAttribute("data-ln-value"), v = i.getAttribute("data-ln-date"), w = i.getAttribute("datetime"), E = null;
    g !== null && g !== "" ? E = g : w !== null && w !== "" ? E = w : v !== null && v !== "" && v !== "true" && !p.test(v) ? E = v : E = i.textContent.trim();
    let A = r(E) || t(E);
    if (!A && E)
      if (isNaN(E))
        A = new Date(E);
  ...
  function e(i) {
    ...
    const g = this, v = i.value, w = i.name, A = (i.closest(".form-element, form") || i.parentNode).querySelectorAll("[data-ln-date-dict]");
    ...
    if (D.type = "button", D.setAttribute("aria-label", i.getAttribute("data-ln-date-label") || "Open date picker"), D.innerHTML = '<svg class="ln-icon" aria-hidden="true"><use href="#ln-icon-calendar"></use></svg>', x.insertAdjacentElement("afterend", D), this._btn = D, this._lastISO = "", Object.defineProperty(q, "value", {
      ...
      if (g._lastISO) {
        ...
        if (N) {
          const B = g.dom.getAttribute(d) || "", z = G(g.dom);
          if (k === o(N, B, z)) return;
        }
      }
      const M = t(k);
  ...
  function t(i) {
    ...
    let E, A, L;
    g === "." ? (E = w[0], A = w[1], L = w[2]) : g === "/" ? (A = w[0], E = w[1], L = w[2]) : v[0].length === 4 ? (L = w[0], A = w[1], E = w[2]) : (E = w[0], A = w[1], L = w[2]), L < 100 && (L += L < 50 ? 2e3 : 1900);
  ...
  e.prototype._openPicker = function() {
    ...
    if (!this.dom[a]) return;
    if (this.isTextElement) {
      S(this.dom, "ln-date:destroyed", { target: this.dom }), delete this.dom[a];
      return;
    }
...
(function() {
  ...
  b.prototype.update = function() {
    ...
    const s = Array.from(this.dom.querySelectorAll("a")), m = window.location.pathname, o = _(m);
    for (const l of s) {
    ...
    h !== -1 && y.splice(h, 1), S(this.dom, "ln-nav:destroyed", { target: this.dom }), delete this.dom[a];
  };
  function _(h) {
    try {
  ...
  function p(h, s) {
    ...
    if (m) {
      ...
      } else s === "data-ln-nav-exact" && (m.exact = h.hasAttribute("data-ln-nav-exact"));
      m.update();
    }
  }
  H(d, a, b, "ln-nav", {
...
(function() {
  ...
  function _() {
    ...
    for (const m of this.tabs) {
      ...
      const o = function(l) {
        ...
        const c = y(m, s.nsKey);
        c && (u && !$t(l) || (s.hashEnabled ? ht(s.nsKey) === c ? s.dom.setAttribute("data-ln-tabs-active", c) : st(s.nsKey, c) : s.dom.setAttribute("data-ln-tabs-active", c)));
      };
      m.addEventListener("click", o), m[a + "Trigger"] = o, s._clickHandlers.push({ el: m, handler: o });
    }
    if (this._onRequestSelect = function(m) {
      ...
      const m = ht(s.nsKey);
      s.dom.setAttribute("data-ln-tabs-active", m !== null ? m : s.defaultKey);
    }, this.hashEnabled)
  ...
  b.prototype._applyActive = function(p) {
    ...
    for (const s in this.mapTabs) {
      const m = this.mapTabs[s];
      s === p ? (m.setAttribute("data-active", ""), m.setAttribute("aria-selected", "true")) : (m.removeAttribute("data-active"), m.setAttribute("aria-selected", "false"));
    }
    for (const s in this.mapPanels) {
      const m = this.mapPanels[s], o = s === p;
...
(function() {
  ...
  b.prototype.open = function() {
    ...
    this.dom[a] && (this.dom.removeEventListener("ln-toggle:request-open", this._onRequestOpen), this.dom.removeEventListener("ln-toggle:request-close", this._onRequestClose), this.dom.removeEventListener("ln-toggle:request-toggle", this._onRequestToggle), S(this.dom, "ln-toggle:destroyed", { target: this.dom }), delete this.dom[a]);
  };
  function _(p) {
    const h = p[a];
    if (!h) return;
  ...
  document.addEventListener("click", function(p) {
    ...
    if (h) {
      const s = h.getAttribute("data-ln-toggle-for"), m = document.getElementById(s);
      if (m && m[a]) {
        p.preventDefault();
...
(function() {
  const d = "data-ln-popover", a = "lnPopover", y = "data-ln-popover-for", b = "data-ln-popover-position";
  if (window[a] !== void 0) return;
  const _ = [];
  let p = null;
  function h() {
  ...
  m.prototype.open = function(l) {
    ...
    l !== -1 && _.splice(l, 1), s(), this._previousFocus && this.trigger && this._previousFocus === this.trigger ? this.trigger.focus() : this.trigger && document.activeElement === document.body && this.trigger.focus(), this._previousFocus = null, S(this.dom, "ln-popover:close", {
      popoverId: this.dom.id,
      target: this.dom,
      trigger: this.trigger
    }), this.trigger = null;
  }, m.prototype.destroy = function() {
    this.dom[a] && (this.dom.removeEventListener("ln-popover:request-open", this._onRequestOpen), this.dom.removeEventListener("ln-popover:request-close", this._onRequestClose), this.dom.removeEventListener("ln-popover:request-toggle", this._onRequestToggle), this.isOpen && this._applyClose(), delete this.dom[a], S(this.dom, "ln-popover:destroyed", {
      popoverId: this.dom.id,
      target: this.dom
    }));
  };
...
(function() {
  const d = "data-ln-tooltip-enhance", a = "data-ln-tooltip", y = "data-ln-tooltip-position", b = "lnTooltipEnhance", _ = "ln-tooltip-portal";
  if (window[b] !== void 0) return;
  let p = 0, h = null, s = null, m = null, o = null, l = null, u = null;
  function c() {
  ...
  function r(i) {
    ...
    w.className = "ln-tooltip", w.textContent = g, i[b + "Uid"] || (p += 1, i[b + "Uid"] = "ln-tooltip-" + p), w.id = i[b + "Uid"], h.appendChild(w);
    const E = w.offsetWidth, A = w.offsetHeight, L = i.getBoundingClientRect(), q = i.getAttribute(y) || "top", x = Rt(L, { width: E, height: A }, q, 6);
    w.style.top = x.top + "px", w.style.left = x.left + "px", w.setAttribute("data-ln-tooltip-placement", x.placement), l ? i.setAttribute("aria-describedby", l + " " + w.id) : i.setAttribute("aria-describedby", w.id), s = w, m = i, f();
  }
  function t() {
    if (!s) {
      e();
    ...
    m && (l !== null ? m.setAttribute("aria-describedby", l) : m.removeAttribute("aria-describedby"), l = null, o !== null && m.setAttribute("title", o)), o = null, s.parentNode && s.parentNode.removeChild(s), s = null, m = null, h && typeof h.hidePopover == "function" && h.matches(":popover-open") && h.hidePopover(), e();
  }
  function n(i) {
...
(function() {
  ...
  function u(e) {
    ...
    const v = e.getAttribute(d) || "/files/upload", w = e.getAttribute(_) || "", E = e.getAttribute("data-ln-upload-delete") || (v.includes("/upload") ? v.replace(/\/upload\/?$/, "/{id}") : v + "/{id}"), A = /* @__PURE__ */ new Map();
    let L = 0;
    function q() {
    ...
    function x(F) {
      ...
      const V = "file-" + ++L, Y = m(F.name), Et = o(Y), at = ft(e, "ln-upload-item", "ln-upload");
      if (!at) return;
      ...
      ot && (ot.disabled = !0), n.appendChild(nt);
      ...
      T.upload.addEventListener("progress", function(R) {
        ...
        if (T.status >= 200 && T.status < 300) {
          ...
          it(nt, { sizeText: s(R.size || F.size), uploading: !1 }), ot && (ot.disabled = !1), A.set(V, {
            serverId: R.id,
            name: R.name,
            size: R.size
      ...
      function I(R) {
        wt && (wt.style.width = "100%"), it(nt, { sizeText: r.error || "Error", uploading: !1, error: !0 }), ot && (ot.disabled = !1), S(e, "ln-upload:error", {
          file: F,
          message: R
        }), S(window, "ln-toast:enqueue", {
          type: "error",
    ...
    function k(F) {
      const V = A.get(F), Y = n.querySelector('[data-file-id="' + F + '"]');
      if (!V || !V.serverId) {
      ...
      fetch(Et, {
        ...
        at.status === 200 ? (Y && Y.remove(), A.delete(F), D(), S(e, "ln-upload:removed", {
          localId: F,
          serverId: V.serverId
    ...
    t.addEventListener("click", N), g.addEventListener("change", B), t.addEventListener("dragenter", z), t.addEventListener("dragover", j), t.addEventListener("dragleave", U), t.addEventListener("drop", rt), n.addEventListener("click", vt), e.lnUploadAPI = {
      getFileIds: function() {
      ...
      clear: function() {
        ...
        A.clear(), n.innerHTML = "", D(), S(e, "ln-upload:cleared", {});
      },
      destroy: function() {
        t.removeEventListener("click", N), g.removeEventListener("change", B), t.removeEventListener("dragenter", z), t.removeEventListener("dragover", j), t.removeEventListener("dragleave", U), t.removeEventListener("drop", rt), n.removeEventListener("click", vt), A.clear(), n.innerHTML = "", D(), delete e.lnUploadAPI;
      }
    };
  }
  function c() {
...
(function() {
  ...
  function y(s) {
    ...
    s.target = "_blank";
    const m = (s.rel || "").split(/\s+/).filter(Boolean);
    m.includes("noopener") || m.push("noopener"), m.includes("noreferrer") || m.push("noreferrer"), s.rel = m.join(" ");
  ...
  function b(s) {
    s = s || document.body;
    for (const m of s.querySelectorAll("a, area"))
      y(m);
  }
  function _() {
    ct(function() {
      document.body.addEventListener("click", function(s) {
        const m = s.target.closest("a, area");
...
(function() {
  ...
  function o(n) {
    n[a + "Row"] || !n.querySelector("a") || (n[a + "Row"] = !0, n._lnLinkClick = function(g) {
      h(n, g);
  ...
  function l(n) {
    n[a + "Row"] && (n._lnLinkClick && n.removeEventListener("click", n._lnLinkClick), n._lnLinkEnter && n.removeEventListener("mouseenter", n._lnLinkEnter), n.removeEventListener("mouseleave", m), delete n._lnLinkClick, delete n._lnLinkEnter, delete n[a + "Row"]);
  }
  function u(n) {
    if (!n[a + "Init"]) return;
    const i = n.tagName;
...
(function() {
  ...
  function b() {
    const h = this, s = new MutationObserver(function(m) {
      for (const o of m)
...
(function() {
  const d = "data-ln-filter", a = "lnFilter", y = "data-ln-filter-key", b = "data-ln-filter-value", _ = "data-ln-filter-hide", p = "data-ln-filter-reset", h = "data-ln-filter-col", s = /* @__PURE__ */ new WeakMap();
  if (window[a] !== void 0) return;
  function m(f) {
  ...
  function u(f) {
    ...
    const E = e.querySelector("[" + y + "]"), A = E ? E.getAttribute(y) : e.getAttribute("data-ln-filter-key") || "col" + r;
    for (let L = 0; L < v.length; L++) {
      const q = t.content.cloneNode(!0), x = q.querySelector("input");
      x && (x.setAttribute(y, A), x.setAttribute(b, v[L]), Lt(q, { text: v[L] }), e.appendChild(q));
    }
  }
  function c(f) {
  ...
  c.prototype._attachHandlers = function() {
    ...
    this.inputs.forEach(function(e) {
      e[a + "Bound"] || (e[a + "Bound"] = !0, e._lnFilterChange = function() {
        if (m(e)) {
    ...
    if (f.colIndex !== null)
      ...
      for (let g = 0; g < i.length; g++) {
        ...
        const w = v.getAttribute("data-" + e.key);
        v.removeAttribute(_), w !== null && t.indexOf(w.toLowerCase()) === -1 && v.setAttribute(_, "true");
      }
    }
    ...
    if (t && n.length > 0) {
      ...
      for (let A = 0; A < n.length; A++)
        E.push(n[A].toLowerCase());
      i[t] = { col: this.colIndex, values: E };
    } else t && delete i[t];
    const g = Object.keys(i), v = g.length > 0, w = r.tBodies;
    ...
    if (this.dom[a]) {
      if (this.colIndex !== null) {
        ...
        if (f) {
          ...
          if (e && s.has(e)) {
            const r = s.get(e), t = this._filterKey;
            t && r[t] && delete r[t], Object.keys(r).length === 0 && s.delete(e);
          }
        }
      }
...
(function() {
  const d = "data-ln-search", a = "lnSearch", y = "data-ln-search-hide";
  if (window[a] !== void 0) return;
  function _(p) {
...
(function() {
  const d = "data-ln-sort", a = "lnSort", y = "data-ln-sort-field", b = "data-ln-sort-state", _ = "data-ln-sort-dir", p = "data-ln-sort-items";
  if (window[a] !== void 0) return;
  function h(o, l) {
    if (l) {
...
(function() {
  ...
  function f(e) {
    ...
    return this._onSetSearch = function(n) {
      ...
      if (!g) return;
      const v = i.getAttribute("data-ln-table-row-action"), w = g.getAttribute("data-ln-table-row-id"), E = g._lnRecord || {};
    ...
    }, e.addEventListener("ln-sort:change", this._onSort), this._onSearchChange = function(n) {
      n.preventDefault(), t._onSetSearch(n);
    }, e.addEventListener("ln-search:change", this._onSearchChange)), this;
  }
  f.prototype._parseRows = function() {
    ...
    for (let t = 0; t < e.length; t++) {
      ...
      for (let E = 0; E < n.cells.length; E++) {
        const A = n.cells[E], L = A.textContent.trim();
        i[E] = xt(A), g[E] = L.toLowerCase(), E < n.cells.length - 1 && v.push(L.toLowerCase());
      }
      let w = null;
    ...
    if (this.isDataDriven) {
      ...
      this._filteredData.sort(function(E, A) {
        return It(E[n], A[n], w, m) * g;
      });
    } else {
      const e = this._searchTerm, r = this._columnFilters, t = Object.keys(r).length > 0, n = this.ths, i = {};
      if (t)
        for (let A = 0; A < n.length; A++) {
          const L = n[A].getAttribute("data-ln-table-filter-col");
          L && (i[L] = A);
        }
      if (!e && !t ? this._filteredData = this._data.slice() : this._filteredData = this._data.filter(function(A) {
        if (e && A.searchText.indexOf(e) === -1) return !1;
        if (t)
          for (const L in r) {
            const q = i[L];
      ...
      const g = this._sortCol, v = this._sortDir === "desc" ? -1 : 1, w = this._filteredData.map(function(A) {
        return A.values[g];
      }), E = Dt(w);
      this._filteredData.sort(function(A, L) {
        return It(A.values[g], L.values[g], E, m) * v;
      });
    }
    ...
    let w = Math.max(0, Math.floor(g / t) - 15);
    w = Math.min(w, r);
    ...
    v = Math.min(v, r);
    const w = Math.min(v + Math.ceil(g / e) + 30, r), E = this.ths.length || 1, A = v * e, L = (r - w) * e, q = document.createDocumentFragment();
    if (A > 0) {
  ...
  function b(_) {
    ...
    _._handlers = {
      ...
      search: function(h) {
        ...
        h.preventDefault();
        const m = h.detail && h.detail.term != null ? h.detail.term : "", o = s.lnTable.name || s.id;
        S(s, "ln-table:set-search", { query: m, term: m, table: o });
      },
      ...
      filter: function(h) {
        ...
        const m = h.detail.key, o = h.detail.values || [], l = s.lnTable.name || s.id, u = s.querySelectorAll("th");
        for (let c = 0; c < u.length; c++)
          if (u[c].getAttribute("data-ln-table-filter-col") === m) {
            const f = u[c].querySelector("[data-ln-table-col-filter]");
            f && f.classList.toggle("ln-filter-active", o.length > 0);
            break;
          }
        S(s, "ln-table:set-filter", { key: m, values: o, table: l });
      },
      ...
      clear: function(h) {
        ...
        if (!s) return;
        const m = s.closest("[data-ln-table]") || p.querySelector("[data-ln-table]");
        if (!m || !m.lnTable) return;
        const o = m.lnTable.name || m.id, l = m.querySelectorAll("th");
        for (let e = 0; e < l.length; e++) {
          const r = l[e].querySelector("[data-ln-table-col-filter]");
          r && r.classList.remove("ln-filter-active");
        }
        const u = m.id, c = u && p.querySelector('[data-ln-search="' + u + '"]') || p.querySelector("[data-ln-search]");
        if (c) {
          const e = c.tagName === "INPUT" || c.tagName === "TEXTAREA" ? c : c.querySelector("input");
          e && (e.value = "");
        }
        const f = u && p.querySelectorAll('[data-ln-filter="' + u + '"]') || p.querySelectorAll("[data-ln-filter]");
        for (let e = 0; e < f.length; e++) {
          const r = f[e].querySelector("[data-ln-filter-reset]");
          r && (r.checked = !0, r.dispatchEvent(new Event("change", { bubbles: !0 })));
        }
        S(m, "ln-table:request-clear-filters", { table: o });
      }
...
(function() {
  ...
  function f(e) {
    ...
    return this._selectable = e.hasAttribute("data-ln-list-selectable"), this._selectableActive = !1, this._selectable && this._enableSelection(), this.isDataDriven ? (this.isLoaded = !1, this.totalCount = 0, this.visibleCount = 0, this.currentSort = null, this.currentFilters = {}, this.currentSearch = "", this._windowed = !1, this._cache = null, this.isDataDriven && e.hasAttribute("data-ln-list-window") && this._enterWindowedMode(), this._lastTotal = 0, this._lastFiltered = 0, this._onSetData = function(t) {
      ...
      if (!i) return;
      const g = n.getAttribute("data-ln-item-action"), v = i.getAttribute("data-ln-item-id"), w = i._lnRecord || {};
  ...
  f.prototype._parseChildren = function() {
    ...
    if (this.isDataDriven) {
      ...
      const n = this.currentSort.field, i = this.currentSort.direction === "desc" ? -1 : 1, g = this._filteredData.map(function(E) {
        return E[n];
      }), v = Dt(g), w = typeof Intl < "u" ? new Intl.Collator(G(this.dom), { sensitivity: "base" }) : null;
      this._filteredData.sort(function(E, A) {
        return It(E[n], A[n], v, w) * i;
      });
    } else {
    ...
    let i, g;
    if (n) {
      const z = this.tbody.getBoundingClientRect(), j = n.getBoundingClientRect(), U = n === this.tbody ? 0 : z.top - j.top + n.scrollTop;
      i = n.scrollTop - U, g = n.clientHeight;
    } else {
    ...
    q = Math.min(q, L);
    const x = Math.ceil(g / A) + 30, D = Math.min(q + x, L), k = Math.min(q * w, r), M = Math.min(D * w, r);
    ...
    if (r) {
      ...
      t = r.scrollTop - rt, n = r.clientHeight;
    } else {
      const U = this.tbody.getBoundingClientRect().top + window.scrollY;
      t = window.scrollY - U, n = window.innerHeight;
    }
    const i = this._readGridLayout(), g = i.columns, v = i.rowGap, w = e + v, E = this._cache.logicalTotal, A = Math.ceil(E / g);
    let L = Math.max(0, Math.floor(t / w) - 15);
    L = Math.min(L, A);
    const q = Math.ceil(n / w) + 30, x = Math.min(L + q, A), D = Math.min(L * g, E), k = Math.min(x * g, E), M = L * w, N = (A - x) * w, B = document.createDocumentFragment();
    if (M > 0) {
    ...
    if (this.isDataDriven) {
      ...
      if (e = ft(this.dom, i, "ln-list"), !e) {
        ...
        if (g) {
          const v = n ? "search" : "initial", w = g.content.querySelector(`[data-ln-empty-when="${v}"]`) || g.content.firstElementChild;
          w && (e = document.importNode(w, !0));
        }
      }
    } else {
    ...
    if (!t) return null;
    if (Lt(t, e), it(t, e), t._lnRecord = e, e.id != null && (t.setAttribute("data-ln-item-id", e.id), this._selectable && this.selectedIds.has(String(e.id)))) {
...
(function() {
  ...
  b.prototype.enable = function() {
    ...
    p.preventDefault(), h.setPointerCapture(p.pointerId), this._dragging = s, s.classList.add("ln-sortable--dragging"), s.setAttribute("aria-grabbed", "true"), this.dom.classList.add("ln-sortable--active"), S(this.dom, "ln-sortable:drag-start", {
      item: s,
      index: o
    });
    ...
    if (!this._dragging) return;
    const h = this._dragging, s = Array.from(this.dom.children), m = s.indexOf(h);
    let o = null, l = null;
    for (const u of s) {
  ...
  function _(p) {
    ...
    s !== h.isEnabled && (h.isEnabled = s, S(p, s ? "ln-sortable:enabled" : "ln-sortable:disabled", { target: p }));
  }
  H(d, a, b, "ln-sortable", {
    onAttributeChange: _
  });
...
(function() {
  ...
  function p(h) {
    ...
    return this._onClick = function(m) {
      if (_("click handler, confirming:", s.confirming, "submitted:", s._submitted, "target:", m.target), !s.confirming)
...
(function() {
  ...
  }, H(d, a, b, "ln-translations");
})();
(function() {
  const d = "data-ln-autosave", a = "lnAutosave", y = "data-ln-autosave-clear", b = "data-ln-autosave-debounce-input", _ = "ln-autosave:";
  if (window[a] !== void 0) return;
  function h(l) {
    const u = s(l);
    if (!u) {
...
(function() {
  ...
  m.prototype._syncToTextarea = function() {
    ...
    for (let v = 0; v < g.length; v++) {
      const w = g[v], E = w.getAttribute("data-ln-editor-action");
      let A = !1;
      if (b[E])
        try {
      ...
      else E === "link" && (A = !!l(n.anchorNode, "A", this._surface));
      s(E) && w.setAttribute("aria-pressed", String(A)), A ? w.classList.add("ln-editor-active") : w.classList.remove("ln-editor-active");
    }
    ...
    i && i.remove(), S(this.dom, "ln-editor:destroyed", { target: this.dom }), delete this.dom[a];
  };
  function o(t) {
    const n = window.getSelection();
...
(function() {
  ...
  function y(_) {
    ...
    for (const s in h) {
      ...
      const m = s.slice(6);
      m && (p[m.charAt(0).toLowerCase() + m.slice(1)] = h[s]);
    }
    return p;
  }
  function b(_, p) {
    ...
    for (let m = 0; m < s.length; m++) {
      const o = s[m].getAttribute("data-ln-fill-form");
      if (o) {
        const l = document.getElementById(o);
        if (l && _.contains(l)) return s[m];
      }
    }
    return s[0];
  }
  document.addEventListener("click", function(_) {
    ...
    const s = p.getAttribute("data-ln-fill-form"), m = document.getElementById(s);
    if (!m) return;
...
(function() {
  ...
  function b(_) {
    ...
    const h = _.getAttribute(d), s = p.elements[h];
    if (!s)
...
(function() {
  ...
  function i(E) {
    return this.dom = E, n(this), E.getAttribute(d) === "relative" && (s.add(this), o()), this;
  }
  ...
  function g(E) {
    const A = E[a];
    if (!A) return;
    E.getAttribute(d) === "relative" ? (s.add(A), o()) : (s.delete(A), s.size === 0 && l()), n(A);
  }
  function v(E) {
    E.nodeType === 1 && E.hasAttribute && E.hasAttribute(d) && E[a] && n(E[a]);
  }
  function w() {
    new MutationObserver(function() {
...
(function() {
  ...
  function q(C) {
    ...
    return this._searchFields = R.split(",").map((O) => O.trim()).filter(Boolean), this._handlers = null, this.isLoaded = !1, this.isInitialized = !1, this.initializationError = null, this.hasCache = !1, this.isSyncing = !1, this.lastSyncedAt = null, this.totalCount = 0, this.presenters = null, this._mutationChain = Promise.resolve(), s[this._name] = this, x(this), this.ready = U(this), this;
  }
  function x(C) {
    C._handlers = {
  ...
  q.prototype.getAll = function(C = {}) {
    ...
    return n(this._name).then((I) => wt(I, C, T));
    ...
    delete s[this._name], delete this.dom[a], S(this.dom, "ln-data-store:destroyed", { store: this._name });
  };
  function mt() {
    return c().then((C) => {
      if (!C) return;
  ...
  H(d, a, q, "ln-data-store"), window[a].clearAll = mt, window[a].init = window[a], window[a].setStorageKey = Zt, typeof window < "u" && (window.lnCore = window.lnCore || {}, window.lnCore.setStorageKey = Zt);
})();
(function() {
  const d = "data-ln-api-connector", a = "lnApiConnector", y = "lnConnector";
  if (window[a] !== void 0) return;
  function b(s) {
    return s.ok ? s.status === 204 ? null : s.json() : s.json().catch(() => null).then((m) => {
      const o = new Error("HTTP " + s.status + ": " + s.statusText);
      throw o.status = s.status, o.data = m, o;
    });
  }
  function _(s) {
    return this.dom = s, s[a] = this, s[y] = this, this.refreshConfig(), this._handlers = null, p(this), this;
  }
  _.prototype.refreshConfig = function() {
    ...
    const m = s.getAttribute("data-ln-api-headers") || "";
    ...
    return s && (m["Idempotency-Key"] = s), m;
  }, _.prototype.fetchDelta = function(s) {
    const m = this;
    let o = J(m.baseUrl, m.path);
    return s != null && s !== "" && (o += (o.indexOf("?") !== -1 ? "&" : "?") + "since=" + encodeURIComponent(s)), window.fetch(o, { method: "GET", headers: m._reqHeaders(), credentials: m.credentials }).then(b);
    ...
    s.search && u.append(l.search, s.search), s.offset != null && u.append(l.offset, s.offset), s.limit != null && u.append(l.limit, s.limit), s.sort && s.sort.field && s.sort.direction && (u.append(l.sortField, s.sort.field), u.append(l.sortDir, s.sort.direction)), s.filters && typeof s.filters == "object" && Object.keys(s.filters).forEach((f) => {
      const e = s.filters[f];
  ...
  }, _.prototype.create = function(s, m, o) {
    const l = this;
  ...
  }, _.prototype.update = function(s, m, o, l, u) {
    const c = this;
    o != null && (m = Object.assign({}, m, { expected_version: o }));
    const f = l ? J(c.baseUrl, l) : J(c.baseUrl, c.path, s);
  ...
  }, _.prototype.delete = function(s, m, o) {
    const l = this;
    return window.fetch(J(l.baseUrl, m || l.path, s), {
      method: "DELETE",
      headers: l._reqHeaders(o),
      credentials: l.credentials
    }).then(b);
  }, _.prototype.bulkDelete = function(s, m, o) {
    const l = this;
  ...
  function p(s) {
    s._handlers = {
      sync: function(o) {
        const l = o.detail || {};
        s.fetchDelta(l.since).then(function(u) {
          S(s.dom, "ln-api-connector:fetched", { data: u, since: l.since, meta: l.meta || null });
        }).catch(function(u) {
      ...
      create: function(o) {
        const l = o.detail || {};
        s.create(l.data, l.url, l.idempotencyKey).then(function(u) {
          const c = u && u.content !== void 0 ? u.content : u, f = u && u.message ? u.message : null;
          S(s.dom, "ln-api-connector:created", { record: c, tempId: l.tempId, message: f, meta: l.meta || null });
        }).catch(function(u) {
          S(s.dom, "ln-api-connector:error", {
            action: "create",
            error: u.message,
      ...
      update: function(o) {
        const l = o.detail || {};
        s.update(l.id, l.data, l.expected_version, l.url, l.idempotencyKey).then(function(u) {
          const c = u && u.content !== void 0 ? u.content : u, f = u && u.message ? u.message : null;
          S(s.dom, "ln-api-connector:updated", { record: c, id: l.id, message: f, meta: l.meta || null });
        }).catch(function(u) {
          S(s.dom, "ln-api-connector:error", {
            action: "update",
            error: u.message,
      ...
      delete: function(o) {
        const l = o.detail || {};
        s.delete(l.id, l.url, l.idempotencyKey).then(function(u) {
          const c = u && u.message ? u.message : null;
          S(s.dom, "ln-api-connector:deleted", { response: u, id: l.id, message: c, meta: l.meta || null });
        }).catch(function(u) {
          S(s.dom, "ln-api-connector:error", {
            action: "delete",
            error: u.message,
      ...
      bulkDelete: function(o) {
        const l = o.detail || {};
        s.bulkDelete(l.ids, l.url, l.idempotencyKey).then(function(u) {
          const c = u && u.message ? u.message : null;
          S(s.dom, "ln-api-connector:bulk-deleted", { response: u, ids: l.ids, message: c, meta: l.meta || null });
        }).catch(function(u) {
          S(s.dom, "ln-api-connector:error", {
            action: "bulk-delete",
            error: u.message,
  ...
  _.prototype.destroy = function() {
    if (!this.dom[a]) return;
    const s = this;
    s._handlers && (["ln-api-connector", "ln-rest-connector"].forEach(function(o) {
    ...
    }), s._handlers = null), S(this.dom, "ln-api-connector:destroyed", { target: this.dom }), delete this.dom[a], delete this.dom[y];
  };
  function h(s) {
    const m = s[a];
    m && m.refreshConfig();
...
(function() {
  ...
  function _(c) {
    return this.dom = c, c[a] = this, c[y] = this, this.refreshConfig(), this._handlers = null, l(this), this;
  }
  _.prototype.refreshConfig = function() {
    const c = this.dom;
  ...
  _.prototype.update = function(c, f, e) {
    return s(this, c, f, e).then((r) => r.record);
  };
  function m(c, f, e, r) {
  ...
  function l(c) {
    c._handlers = {
      ...
      update: function(e) {
        ...
        r.expected_version !== void 0 && (t._rev = r.expected_version), s(c, r.id, t, r.idempotencyKey).then(function(n) {
  ...
  _.prototype.destroy = function() {
    ...
    }), c._handlers = null), S(this.dom, "ln-couchdb-connector:destroyed", { target: this.dom }), delete this.dom[a], delete this.dom[y];
  };
  function u(c) {
    const f = c[a];
    f && f.refreshConfig();
...
function Ht(d, a, y) {
  ...
  return a && (y || b || !d.isLoaded) ? "remote" : d && !d.initializationError ? "store" : "none";
}
class Ke {
  constructor() {
  ...
  wait(a) {
    return a ? this._pending.has(a) ? Promise.reject(new Error(`Duplicate mutation requestId: ${a}`)) : new Promise((y, b) => {
      this._pending.set(a, { resolve: y, reject: b });
  ...
  _settle(a, y) {
    const b = a && a.requestId;
    if (!b) return !1;
    const _ = this._pending.get(b);
    return _ ? (this._pending.delete(b), y ? _.reject(a.error || new Error("Store mutation failed")) : _.resolve(a), !0) : !1;
  }
}
(function() {
  const d = "data-ln-data-coordinator", a = "lnDataCoordinator", y = "lnCoordinator", b = "data-ln-form-scope";
  if (window[a] !== void 0) return;
  const _ = /* @__PURE__ */ new Set();
  let p = !1, h = null, s = null, m = null;
  function o() {
    p || (p = !0, h = function() {
    ...
    }, window.addEventListener("online", h), window.addEventListener("offline", s), document.addEventListener("visibilitychange", m));
  }
  function l() {
    p && (_.size > 0 || (window.removeEventListener("online", h), window.removeEventListener("offline", s), document.removeEventListener("visibilitychange", m), h = null, s = null, m = null, p = !1));
  }
  function u() {
    try {
  ...
  f.prototype._parseStaleAttributes = function() {
    ...
    const i = t.data || {}, g = i.id, v = i.expected_version, w = Object.assign({}, i);
    delete w.id, delete w.expected_version;
    const E = t.method.toUpperCase();
    E === "POST" ? this._fanOutCreate(n, w, t.action) : (E === "PUT" || E === "PATCH") && this._fanOutUpdate(n, g, w, v, t.action);
  ...
  function e(t) {
    t._handlers = {
      ...
      queueSend: function(n) {
        ...
        const g = n.detail || {}, v = g.entryId, w = g.op, E = g.targetId, A = g.payload, L = g.expectedVersion, q = g.meta || {}, x = q.action || null, D = g.idempotencyKey || v;
      ...
      connError: function(n) {
        const i = n.detail || {}, g = i.meta || {}, v = g.op || i.action, w = i.status || 0, E = t.findChildren();
        if (v === "sync") {
  ...
  f.prototype._ownsStore = function(t) {
    ...
    return (x && x.ready ? x.ready : Promise.resolve()).then(function() {
      ...
      if (k === "remote") {
        S(i, "ln-" + n + ":set-loading", { loading: !0 }), S(A.connectorEl, "ln-api-connector:request-query", {
          query: E,
    ...
    const g = this.findChildren(), v = g.store, w = v && v.ready ? v.ready : Promise.resolve(), E = this;
    return w.then(function() {
      const A = Ht(v, g.connector, !1);
      if (A === "remote") {
      ...
      E._reportReconciliationError("options-query", A, { targetEl: n, kind: "options" });
    });
  }, f.prototype._serveStat = function(t) {
    const n = t.target, i = n.getAttribute("data-ln-stat");
    if (!this._ownsStore(i)) return;
    const g = t.detail && t.detail.filters ? t.detail.filters : null, v = this.findChildren(), w = v.store, E = w && w.ready ? w.ready : Promise.resolve(), A = this;
    return E.then(function() {
      ...
      if (L === "store")
        return w.count(g).then(function(q) {
          S(n, "ln-stat:set-count", { count: q });
        });
    }).catch(function(L) {
      A._reportReconciliationError("stat-query", L, { targetEl: n, kind: "stat" });
    });
    ...
    }), document.removeEventListener("ln-table:request-data", t._handlers.reqTableData), document.removeEventListener("ln-list:request-data", t._handlers.reqListData), document.removeEventListener("ln-options:request-data", t._handlers.reqOptions), document.removeEventListener("ln-stat:request-count", t._handlers.reqStat), t.dom.removeEventListener("ln-data-store:ready", t._handlers.refresh), t.dom.removeEventListener("ln-data-store:created", t._handlers.refresh), t.dom.removeEventListener("ln-data-store:updated", t._handlers.refresh), t.dom.removeEventListener("ln-data-store:deleted", t._handlers.refresh), t.dom.removeEventListener("ln-data-store:mutation-error", t._handlers.mutationError), t.dom.removeEventListener("ln-data-store:synced", t._handlers.refreshSynced), t._handlers = null), t._boundQueries = null, t._boundDelivered = null, t._mutationReceipts.close(new Error("Data coordinator destroyed")), t._mutationReceipts = null, _.delete(this), l(), delete this.dom[a], delete this.dom[y];
  };
  function r(t, n) {
    const i = t[a];
...
function Qe(d, a, y, b) {
  ...
  return _.forEach((s, m) => {
    s.sort((l, u) => l.seq - u.seq);
    const o = s[0];
    if (!(!o || o.status === "failed")) {
      ...
      o.status = "inflight", o.leaseOwner = a, o.leaseUntil = b + y, o.updatedAt = b, p.push(o);
    }
  }), { entries: p, wakeups: h };
}
function $e(d, a, y, b, _) {
  const p = [], h = [];
  for (const s of d || []) {
    if (s.entryId === a) {
      h.push(s.entryId);
    ...
    s.chainKey === y && (s.chainKey = b, s.targetId === y && (s.targetId = b), s.meta && s.meta.id === y && (s.meta.id = b), s.meta && typeof s.meta.action == "string" && (s.meta.action = Ge(s.meta.action, y, b)), s.updatedAt = _, p.push(s));
  }
  return { changed: p, deleted: h };
}
class Ye {
  constructor(a) {
    a = a || {}, this.indexedDB = a.indexedDB || globalThis.indexedDB, this.keyRange = a.IDBKeyRange || globalThis.IDBKeyRange, this.dbName = a.dbName || Ve, this.now = a.now || (() => Date.now()), this.uuid = a.uuid || (() => crypto.randomUUID()), this._db = null, this._ready = null;
  }
  open() {
    return this._ready ? this._ready : !this.indexedDB || !this.keyRange ? Promise.resolve(null) : (this._ready = new Promise((a, y) => {
      ...
      }, b.onerror = () => y(et(b, "Queue database open failed")), b.onsuccess = (_) => {
  ...
  deleteDatabase() {
    return this.close(), this.indexedDB ? new Promise((a, y) => {
      ...
      b.onsuccess = () => a(), b.onerror = () => y(et(b, "Queue database delete failed")), b.onblocked = () => y(new Error("Queue database delete blocked"));
    }) : Promise.resolve();
  }
  allForScope(a) {
    return this.open().then((y) => y ? new Promise((b, _) => {
      const h = y.transaction(Q, "readonly").objectStore(Q).index("by_scope_seq").getAll(bt(this.keyRange, a));
      h.onsuccess = () => b(h.result || []), h.onerror = () => _(et(h, "Queue scope read failed"));
    }) : []);
  }
  enqueue(a, y) {
    return y = y || {}, this.open().then((b) => b ? new Promise((_, p) => {
      const h = b.transaction([Z, Q], "readwrite"), s = h.objectStore(Z), m = h.objectStore(Q), o = ie(a);
      let l = null;
      const u = (f) => {
        const e = f + 1;
        l = {
      ...
      c.onerror = () => p(et(c, "Queue sequence read failed")), c.onsuccess = () => {
        ...
        const e = m.index("by_scope_seq").getAll(bt(this.keyRange, a));
        e.onerror = () => p(et(e, "Queue sequence migration failed")), e.onsuccess = () => {
  ...
  claimReady(a, y, b) {
    return this.open().then((_) => _ ? new Promise((p, h) => {
      const s = _.transaction(Q, "readwrite"), m = s.objectStore(Q), o = m.index("by_scope_seq").getAll(bt(this.keyRange, a)), l = this.now();
      let u = { entries: [], wakeups: [] };
      o.onerror = () => h(et(o, "Queue claim read failed")), o.onsuccess = () => {
        u = Qe(o.result || [], y, b, l);
        for (const c of u.entries) m.put(c);
      }, s.oncomplete = () => p(u), s.onerror = () => h(s.error || new Error("Queue claim transaction failed")), s.onabort = () => h(s.error || new Error("Queue claim transaction aborted"));
    }) : { entries: [], wakeups: [] });
  }
  ack(a, y) {
    return this._updateEntry(a, y, (b, _) => (_.delete(b.entryId), { status: "acked", entry: b }));
  }
  nack(a, y, b, _) {
    _ = _ || {};
    const p = _.maxAttempts || 8, h = _.backoff || [2e3, 5e3, 15e3, 6e4, 3e5];
    return this.open().then((s) => s ? new Promise((m, o) => {
      const l = s.transaction([Q, Z], "readwrite"), u = l.objectStore(Q), c = l.objectStore(Z), f = u.get(y);
      let e = null;
      f.onerror = () => o(et(f, "Queue nack read failed")), f.onsuccess = () => {
        ...
        if (!(!r || r.scope !== a)) {
          ...
          if (re(r), r.updatedAt = this.now(), b === "auth") {
            r.status = "pending", u.put(r), c.put({ key: Tt(a), value: !0 }), e = { status: "auth", entry: r };
            return;
          }
          if (b === "retry") {
  ...
  remap(a, y, b) {
    return this._remapTransaction(a, null, y, b);
  }
  resolveCreate(a, y, b, _) {
    return this._remapTransaction(a, y, b, _);
  }
  _remapTransaction(a, y, b, _) {
    return this.open().then((p) => p ? new Promise((h, s) => {
      const m = p.transaction(Q, "readwrite"), o = m.objectStore(Q), l = o.index("by_scope_seq").getAll(bt(this.keyRange, a));
      let u = { changed: [], deleted: [] };
      l.onerror = () => s(et(l, "Queue remap read failed")), l.onsuccess = () => {
        u = $e(l.result || [], y, b, _, this.now());
        for (const c of u.deleted) o.delete(c);
        for (const c of u.changed) o.put(c);
      }, m.oncomplete = () => h(u.changed), m.onerror = () => s(m.error || new Error("Queue remap transaction failed")), m.onabort = () => s(m.error || new Error("Queue remap transaction aborted"));
    }) : []);
  }
  resetFailed(a) {
    return this.open().then((y) => y ? new Promise((b, _) => {
      const p = y.transaction(Q, "readwrite"), h = p.objectStore(Q), s = h.index("by_scope_seq").getAll(bt(this.keyRange, a));
      let m = 0;
      s.onerror = () => _(et(s, "Queue failed-entry read failed")), s.onsuccess = () => {
  ...
  getPaused(a) {
    return this.open().then((y) => y ? new Promise((b, _) => {
      const h = y.transaction(Z, "readonly").objectStore(Z).get(Tt(a));
      h.onsuccess = () => b(!!(h.result && h.result.value)), h.onerror = () => _(et(h, "Queue pause-state read failed"));
    }) : !1);
  }
  setPaused(a, y) {
    return this.open().then((b) => {
      if (b)
        return new Promise((_, p) => {
          ...
          h.objectStore(Z).put({ key: Tt(a), value: !!y }), h.oncomplete = () => _(), h.onerror = () => p(h.error || new Error("Queue pause-state write failed")), h.onabort = () => p(h.error || new Error("Queue pause-state write aborted"));
        });
    });
  }
  clear(a) {
    return this.open().then((y) => {
      if (y)
        return new Promise((b, _) => {
          const p = y.transaction([Q, Z], "readwrite"), s = p.objectStore(Q).index("by_scope_seq").openCursor(bt(this.keyRange, a));
          s.onsuccess = (m) => {
            const o = m.target.result;
            o && (o.delete(), o.continue());
          }, s.onerror = () => _(et(s, "Queue clear failed")), p.objectStore(Z).delete(ie(a)), p.objectStore(Z).delete(Tt(a)), p.oncomplete = () => b(), p.onerror = () => _(p.error || new Error("Queue clear transaction failed")), p.onabort = () => _(p.error || new Error("Queue clear transaction aborted"));
        });
    });
  }
  _updateEntry(a, y, b) {
    return this.open().then((_) => _ ? new Promise((p, h) => {
      const s = _.transaction(Q, "readwrite"), m = s.objectStore(Q), o = m.get(y);
      let l = null;
...
(function() {
  const d = "data-ln-api-queue", a = "lnApiQueue", y = [2e3, 5e3, 15e3, 6e4, 3e5], b = 8, _ = 6e4;
  if (window[a] !== void 0) return;
  function p() {
    try {
      return crypto.randomUUID();
...
(function() {
  ...
  y.prototype._rebuild = function(b) {
    const _ = this.dom, p = this._valueField, h = this._labelField, s = _.value, m = _.querySelectorAll("option");
...
(function() {
  ...
  function y(_) {
    ...
    const p = _.indexOf(":");
    if (p === -1) return null;
    const h = _.slice(0, p), s = _.slice(p + 1), m = {};
    return m[h] = [s], m;
  }
  function b(_) {
...
(function() {
  const d = "ln-icon-sprite", a = "#ln-icon-", y = "#ln-icon-custom-", b = /* @__PURE__ */ new Set(), _ = /* @__PURE__ */ new Set();
  let p = null;
  const h = (window.LN_ICON_CDN || "https://cdn.jsdelivr.net/npm/@tabler/icons@3.31.0/icons/outline").replace(/\/$/, ""), s = (window.LN_ICON_CUSTOM_CDN || "").replace(/\/$/, ""), m = "lni:", o = "lni:v", l = "1";
  function u() {
    try {
  ...
  function f(i) {
    return i.indexOf(y) === 0 ? s + "/" + i.slice(y.length) + ".svg" : h + "/" + i.slice(a.length) + ".svg";
  }
  function e(i, g) {
    const v = g.match(/viewBox="([^"]+)"/), w = v ? v[1] : "0 0 24 24", E = g.match(/<svg[^>]*>([\s\S]*?)<\/svg>/i), A = E ? E[1].trim() : "", L = g.match(/<svg([^>]*)>/i), q = L ? L[1] : "", x = document.createElementNS("http://www.w3.org/2000/svg", "symbol");
  ...
  function r(i) {
    ...
    const g = i.slice(1);
    try {
      const w = localStorage.getItem(m + g);
      if (w) {
        e(g, w), b.add(i);
        return;
      }
    ...
    fetch(v).then(function(w) {
      ...
      e(g, w), b.add(i), _.delete(i);
      try {
        localStorage.setItem(m + g, w);
      } catch {
      }
    }).catch(function(w) {
      console.error("[ln-icon] Fetch failed for:", g, w), _.delete(i);
    });
  }
  function t(i) {
    const g = 'use[href^="' + a + '"], use[href^="' + y + '"]', v = i.querySelectorAll ? i.querySelectorAll(g) : [];
    ...
    t(document), new MutationObserver(function(i) {
      i.forEach(function(g) {
        if (g.type === "childList")
          ...
          v && (v.indexOf(a) === 0 || v.indexOf(y) === 0) && r(v);
        }
      });
    }).observe(document.body, {
      childList: !0,
