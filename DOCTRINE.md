...
* **DOM as Public Control Plane:** Every `data-ln-*` attribute on a component's own host element — configuration as much as control state (open/closed, active tab, sort direction, search term, page offset, validation state, window size, stale threshold, or any other authored/written value) — is observed by the one shared attribute observer `ln-core` installs on `document.body` (named exceptions: `ln-progress`, reading its parent; `ln-icon`, bundle-cost floor; `<html lang>`, watched separately by `ensureLocaleObserver`). The attribute in the DOM is the **Single Source of Truth**: a component never keeps a private mirror of a value it also writes to, or reads from, an attribute — it reads the DOM live (`defineAttrs`) and reacts to the shared observer's dispatch (`effects` / `onAttrChange`, or `observeAttributes` for a component that owns its own lifecycle). **Because there is no global mutable state tree, the blast radius of any state bug is strictly contained to the individual component.**
...
* **Attributes Are Set From Outside; Runtime Status Stays Private:** A `data-ln-*` attribute holds state that can be **set from outside** — by the author, a coordinator, or `setAttribute` — and the component obeys it: open/close, sort direction, active tab, an explicit connect/disconnect command. The in-between status a component only **observes** while it works (`connecting`, `open`/`closed` of a connection, `loading`, `uploading`, `dragover`) is private JS state, never an attribute: writing `connected` into an attribute would not connect anything. The component announces each status change with an event; whoever listens — typically the coordinator — decides how to show it (a toast, a debug line, a red/yellow/green chip).
* **State Lives in Attributes, Never in Classes:** State that can be set from outside goes to `data-ln-*` attributes only — never to a CSS class. Classes belong to the replaceable visual layer (theme, Tailwind); a class set admits impossible combinations; and the core observer reacts to attribute names, not classes. Two forms, as in the platform:
  - mutually exclusive values → valued attribute: `data-ln-sort-state="asc"`
...
* **Styling `data-ln-*` Attributes:** `data-ln-*` attributes — hooks and state alike — are ordinary styling targets. Ashlar styles them through tokens (e.g. `[data-ln-sort][data-ln-sort-state="asc"]` in `theme/components/_sort.scss`). Project CSS may override any of it freely.
