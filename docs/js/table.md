...
| `data-ln-table-window="N"` | Wrapper | Opt-in server-side sliding-window virtualization. `N` sets the resident-row cap (`windowSize`, default 1000). Requires Data-Driven Mode. Delegates to `ln-core.createWindowCache`. Observable: adding/removing this attribute on an initialized table enables/disables windowed mode live, without re-init; changing `N` while windowed live-reconfigures `windowSize`. `data-ln-table-window-page`, `data-ln-table-window-threshold`, and `data-ln-table-count` are likewise observable and apply to the live cache. |
...
The windowing attributes and windowed mode itself are both live-observable — no re-init required. Mutating `data-ln-table-window-page`, `data-ln-table-window-threshold`, or `data-ln-table-count` reconfigures the live `createWindowCache` via `configure()`/`setGrandTotal()`. Adding `data-ln-table-window` to an initialized, non-windowed, data-driven table enables windowing live: it seeds the cache from resident rows (honoring `data-ln-table-count` if present, else the resident row count). Removing `data-ln-table-window` disables windowing live: the cache is destroyed and the table issues a fresh full `ln-table:request-data` (no `offset`/`limit`) to repopulate the complete dataset before rendering non-windowed again.
