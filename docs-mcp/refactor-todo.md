...
- **`skills/` рестуктурирање standalone (2026-07-13)** — рекурзивно читање на `skills/` со подфолдери (`skills/app/`, идни `skills/web/`, `skills/wordpress/`); `context` изведен од патеката (не од frontmatter — полето е избришано); skill-фајловите немаат `context:`/`source:` во frontmatter; slug-квалификација по context (`app/ui` vs идно `web/ui`, за колизија на `name:`); `validate_docs` — забрана за надворешни линкови во skills (дозволени само `./` sibling линкови во истиот context subfolder).
