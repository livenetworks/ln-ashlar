...
6. По v2 write-pipeline (2026-07-09): секој документ што допира форми/податочен слој се проверува за `{message,content}` envelope, `data-ln-data-coordinator-scope`, native-submit intake (no custom event) — draft-овите се постари од овие одлуки.
