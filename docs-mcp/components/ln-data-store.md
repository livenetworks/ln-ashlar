...
|---|---|---|---|---|
| `data-ln-data-store` | Root | `String` | *Required* | Declares the store name (also accepts `data-ln-store`). |
...
| `fullReload` | `()` | `Promise<void>` | Clears store cache and metadata, then triggers a full sync. |
...
|---|---|---|---|---|
| `ln-store:request-create` | Listens | No | Optimistically writes a new record. | `{ tempId: String, data: Object }` |
| `ln-store:request-update` | Listens | No | Optimistically updates a record or rekeys ID. | `{ id: ID, data: Object }` |
| `ln-store:request-delete` | Listens | No | Optimistically deletes a record. | `{ id: ID }` |
| `ln-store:request-bulk-delete` | Listens | No | Optimistically deletes multiple records. | `{ ids: Array }` |
| `ln-store:request-remote-sync` | Emits | No | Triggers network sync request. | `{ since: Number\|null }` |
| `ln-store:initialized` | Emits | No | Emitted when store IndexedDB is configured. | `{ store: String, hasCache: Boolean, lastSyncedAt: Number\|null, count: Number }` |
| `ln-store:ready` | Emits | No | Emitted when data is ready for viewing. | `{ store: String, count: Number, source: 'cache'\|'server' }` |
| `ln-store:loaded` | Emits | No | Emitted on initial load sync completion. | `{ store: String, count: Number }` |
| `ln-store:created` | Emits | No | Emitted after optimistic creation is done. | `{ store: String, record: Object, tempId: String }` |
| `ln-store:updated` | Emits | No | Emitted after optimistic update/rekey is done. | `{ store: String, record: Object, previous: Object }` |
| `ln-store:deleted` | Emits | No | Emitted after optimistic deletion is done. | `{ store: String, id: ID }` \| `{ store: String, ids: Array }` |
| `ln-store:synced` | Emits | No | Emitted after subsequent delta sync completes. | `{ store: String, added: Number, deleted: Number, changed: Boolean }` |
| `ln-store:destroyed` | Emits | No | Emitted when the store instance is destroyed. | `{ store: String }` |
| `ln-store:quota-exceeded` | Emits | No | Emitted on `document` if database storage runs out of quota. | `{ error: Error }` |
...
- **Invalidation / versioning:** The store schema version is tracked. If the schema version shifts, the cache store is cleared, database version incremented, and metadata reset.
...
> 2. **Omission of tempId:** Failing to pass a `tempId` in `ln-store:request-create` payload fails because the store has no built-in auto-UUID fallback.
...
sequenceDiagram
    ...
    Store-->>UI: emit ln-store:initialized (cache status report)
    ...
    par Optimistic Write
        ...
        Store-->>UI: emit ln-store:created (UI updates instantly)
    and Remote POST
    ...
    alt Sync Success (2xx)
        ...
        Store-->>UI: emit ln-store:updated (UI maps tempId to 101)
    else Sync Error (4xx)
        ...
        Store-->>UI: emit ln-store:deleted (UI drops temp row)
    end
```
