...
| `renderList` | `(container: HTMLElement, items: Array, templateName: String, keyFn: Function, fillFn: Function, componentTag?: String)` | `void` | Keyed DOM list renderer. Reuses existing nodes matching `data-ln-render-key` to minimize DOM thrashing. |
