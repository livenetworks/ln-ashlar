...
1. **Submit Intake:** `ln-data-coordinator` claims the native submit of forms opted in via `data-ln-data-coordinator-scope` (bubble phase, document level, `preventDefault()`), serializes inputs, and applies egress mapping.
