...
status: stable
domain: frontend
summary: Architectural and semantic authoring doctrines for marketing pages, presentation landing sites, hero sections, and bento grids in ln-ashlar.
source: theme/ln-ashlar-landing.scss, theme/presentation/, demo/landing/index.html, PRESENTATION-DOCTRINE.md
tags: [doctrine, presentation, marketing, landing, semantic, bento, typography]
---

# 🎨 Presentation & Marketing Engineering Doctrine

## Summary

This document defines the architectural guidelines, semantic HTML markup contracts, SCSS mixin composition models, and performance standards for building high-impact presentation websites and marketing landing pages with `ln-ashlar`. It establishes the duality between data-dense application shells and presentation experiences, details the anti-BEM semantic doctrine, explains 3-axis theming in marketing contexts, and outlines compositor-thread animation rules.

---

## 1. The Duality of ln-ashlar: Applications vs. Presentation

`ln-ashlar` powers two distinct architectural experiences on a single unified, zero-dependency DOM-first foundation:

| Architectural Dimension | Application Shell (`ln-ashlar.css`) | Presentation Experience (`ln-ashlar-landing.css`) |
|---|---|---|
| **Primary Focus** | High data density, workflow efficiency, CRUD | Brand elevation, user conversion, storytelling |
...
| **Surface Aesthetics** | Structured flat panels, subtle hairline borders | Glassmorphism, ambient radial glows, asymmetric bento grids |
...
- Identical CSS token cascade and derivation engine.
- Full compatibility with Ashlar's vanilla JS components (`ln-accordion`, `ln-modal`, `ln-tabs`, `ln-popover`, `ln-toggle`).
...
```
Theme islands must rely on Ashlar's CSS custom property cascading. Writing ad-hoc inline styles or hardcoded color overrides inside islands is strictly forbidden.

---

## 3. Pure Semantic HTML5 & The Anti-BEM Doctrine
...
### Structural Rules for Presentation Markup:
...
To eliminate CSS specificity conflicts and support headless/custom semantic markup, all presentation patterns follow the **Dual Authoring Pattern**:
...
- **Interactive FAQs:** Use Ashlar's native [`ln-accordion`](../components/ln-accordion.md) (`<ul data-ln-accordion="single" class="accordion">`) for accessible keyboard-navigable FAQ sections.
- **Lead Capture & Inquiries:** Use [`ln-modal`](../components/ln-modal.md) (`<dialog data-ln-modal>`) for high-converting contact and demo modals.
- **Billing Period Switches:** Use [`ln-tabs`](../components/ln-tabs.md) or accessible button groups for monthly / annual pricing toggles.
- **Live Theme & Appearance Pickers:** Use [`ln-popover`](../components/ln-popover.md) (`popover` API with graceful polyfill) for non-modal floating control panels.
