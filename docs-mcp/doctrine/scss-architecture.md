...
> 3. **Do Not Infer Tonal Direction from Names:** `--bg-sunken` is darker than `--bg-base` in light mode (96% vs 100%) but **lighter** than `--bg-base` in dark mode (12% vs 7%). This is intentional so sunken wells do not punch through the app-shell ground. The dark surfaces sit this low because the three-tier `--fg-*` ladder needs the luminance headroom to clear 7:1; raising them collapses the ladder.
> 4. **Do Not Rebind Vocabulary on Components:** Never write `.card { --bg-base: ... }`. Always write `.card { --color-bg: var(--bg-elevated); }`.
> 5. **Own-the-pair:** Any element that rebinds `--color-bg` and paints `background-color` must also paint `color: var(--color-fg)` in the same rule. A background without a foreground inherits its text colour from an arbitrary ancestor and cannot guarantee contrast against a surface it chose itself. The same obligation runs the other way for fills: rebinding `--brand-primary` obliges rebinding `--color-accent-fg`, because the shipped pairing was solved for the shipped lightness and does not follow a new one.
> 6. **Own-the-pair Has a Scope, and `:root` Is Not All of It:** A consumer rebinding `--color-accent-fg` at `:root` covers the default brand only. The `[data-theme]` presets are **bare selectors at (0,1,0)** declaring that token on the island element itself, so an inherited `:root` value is overridden there — the correction cannot reach inside an island. Overriding a preset's pair means matching its specificity and writing `[data-theme="ocean"] { … }`, not `:root { … }`.
