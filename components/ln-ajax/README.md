...
- **Forms carrying `data-ln-data-coordinator-scope` are skipped entirely** (one-time `console.warn`) — the `ln-data-coordinator` write pipeline takes precedence over ajax progressive enhancement.
...
Source: `components/ln-ajax/ln-ajax.js`.
