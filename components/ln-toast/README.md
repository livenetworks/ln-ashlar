...
[`components/ln-toast/template.html`](template.html) as your starting point — it is a
...
- **Standalone IIFE**: For lightweight pages, load the standalone, self-registering IIFE version:
  ```html
  <script src="components/ln-toast/ln-toast.js" defer></script>
  ```
- **Active Source (ESM)**: Development source is located at [components/ln-toast/src/ln-toast.js](file:///c:/laragon/www/ln-ashlar/components/ln-toast/src/ln-toast.js).

---

## 🔧 Internals

Source: `components/ln-toast/ln-toast.js`. One `_Component` instance per `[data-ln-toast]` container; no reactive proxy, direct DOM manipulation.
