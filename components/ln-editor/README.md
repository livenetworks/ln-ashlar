...
- **Mixin:** `theme/config/mixins/_editor.scss` — `@mixin editor`
- **Component:** `theme/components/_editor.scss` — applies to `[data-ln-editor]`
- **Co-located:** `components/ln-editor/ln-editor.scss` — link popover (runtime-cloned from the authored `<template data-ln-template="ln-editor-link-popover">`)
...
Source: `components/ln-editor/ln-editor.js`. No cached formatting state — every toolbar sync re-queries `document.queryCommandState` and DOM ancestry on `selectionchange`.
