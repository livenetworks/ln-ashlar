...
1. **No cycle logic in JS.** A click reads `data-ln-sort-dir` off the clicked button, writes it to
   ...
   `theme/config/mixins/_sort.scss`.
...
Source: `components/ln-sort/ln-sort.js`. One instance per `[data-ln-sort]`, stored at `element.lnSort`.
...
`theme/config/mixins/_sort.scss` decides which button is even clickable next.
