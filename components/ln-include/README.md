...
- **Boot Gating (No race conditions):** Automatically delays page-wide component initialization (e.g., `ln-table` data fetches) until the initial hosts' fetches have completed.
...
Route views can be loaded the same way, with zero changes to `ln-router` — it still finds `<template data-ln-route>` and clones `.content` as usual:
...
- **Content vs. Registry:** The partial file must contain the *content* of the template itself (e.g. `<tr>...</tr>`), NOT a registry of named `<template>` wrappers. (Nested templates at any depth inside the partial are supported, but wrapping the partial itself in `<template data-ln-template="...">` inside the file creates a dead registry: that outer wrapper is never cloned by anything, so its content never reaches the document.)
...
- **Standalone Bundle Script Order:** If you load individual component files (e.g. `components/ln-table/ln-table.js`) instead of using the master bundle (`components/index.js`), you **must** ensure that `ln-include.js` executes before all other components. Use plain script tags or `defer` (never `async`), otherwise other components will run their DOMContentLoaded sweeps and bypass the gate.
