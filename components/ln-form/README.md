...
- **Standalone IIFE**: For lightweight pages, load the standalone,
  ...
  ```html
  <script src="components/ln-form/ln-form.js" defer></script>
  ```
- **Active Source (ESM)**: Development source is located at
  [components/ln-form/src/ln-form.js](file:///c:/laragon/www/ln-ashlar/components/ln-form/src/ln-form.js).

---

## 🔧 Internals

Source: `components/ln-form/ln-form.js`. Instance state is minimal: `dom` (the form) and `_baseAction` (the `action` attribute captured once at init, restored on "new" mode). No validation index, no debounce, no submit-button bookkeeping — `ln-form` does not orchestrate submission.
