...
Source: `components/ln-table-coordinator/src/ln-table-coordinator.js`. This is a real per-instance component: `_component(dom)` calls `_bindEvents(self)`, which attaches listeners to `self.dom` and cleans them up symmetrically on `destroy()`.
