...
export function generateTokensTable(sources) {
	...
	const rows = [
		...
		row('--border-ui', 'token', `${requireVar(lightVars, '--border-ui', '_palette.scss (ln-values-light)')} (light) / ${requireVar(darkVars, '--border-ui', '_palette.scss (ln-values-dark)')} (dark)`, 'Interactive control boundary — input, select, checkbox. Solved to 3:1 against `--bg-base` per WCAG 1.4.11; decorative borders use `--border-subtle` / `--border-strong` instead'),
		row('--color-ink', 'token', requireVar(colorChainVars, '--color-ink', '_palette.scss (base-defaults block)'), 'Dark foreground pole for fills too light to carry white at 7:1. A polarity-independent literal — never bind it to `--color-neutral-900`, which inverts to near-white in dark mode'),
...
export function generateThemingTable(sources) {
	...
	const rows = [
		...
		row('--bg-base', 'token', `${requireVar(lightVars, '--bg-base', '_palette.scss (ln-values-light)')} (light) / ${requireVar(darkVars, '--bg-base', '_palette.scss (ln-values-dark)')} (dark)`, 'Base canvas background'),
		row('--bg-elevated', 'token', `${requireVar(lightVars, '--bg-elevated', '_palette.scss (ln-values-light)')} (light) / ${requireVar(darkVars, '--bg-elevated', '_palette.scss (ln-values-dark)')} (dark)`, 'Elevated card surface (flat in light, +3% in dark)'),
		row('--bg-sunken', 'token', `${requireVar(lightVars, '--bg-sunken', '_palette.scss (ln-values-light)')} (light) / ${requireVar(darkVars, '--bg-sunken', '_palette.scss (ln-values-dark)')} (dark)`, 'Sunken well surface (darker in light, +5% in dark)'),
