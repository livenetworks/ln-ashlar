...
`ln-modal-fill` е координатор на проектот кој служи како мост помеѓу системот за навигација преку хаш кај модалите (`ln-modal` / `hash.js`) и декларативното пополнување форми (`ln-fill`).
...
*   **`ln-modal`**: Го активира процесот испраќајќи го настанот `ln-modal:open` со соодветните хаш параметри.
*   **`ln-fill`**: Механизмот кој физички ги чита атрибутите од изворниот елемент и ги распределува по полињата.
*   **`ln-form`**: Главниот примател на податоците кој ја врши распределбата во формите и го менува своето RESTful однесување врз основа на тоа дали се работи за креирање или измена.
