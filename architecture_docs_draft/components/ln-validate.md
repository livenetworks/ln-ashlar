...
`ln-validate` е лесен, изолиран примитив за визуелна валидација на кориснички внес (лоциран во [`js/ln-validate/src/ln-validate.js`](../../js/ln-validate/src/ln-validate.js)) кој ја обвиткува нативната `ValidityState` API на прелистувачот. Неговата примарна задача е да врши синхронизација помеѓу моменталната валидност на инпутот и придружните визуелни елементи за прикажување грешки во DOM-от. На ниво на форма, истиот компонент поседува и единствениот submit-гејт: штом формата содржи барем едно валидирано поле, при поднесување (секој метод, без исклучок) се проверуваат сите полиња — невалидна форма се блокира и фокусот оди на првото невалидно поле.
...
> * **НЕ ја презема валидна поднесена форма за write pipeline-от** — блокирањето на невалиден `submit` е сопствена одговорност на `ln-validate` (сопствениот submit-гејт), но клејмувањето на валиден submit за database write е исклучиво на координаторот `ln-data-coordinator`.
...
*   **`ln-form`**: Population/RESTful-routing примитива — воопшто не слуша `submit` и нема улога во валидацијата. Дистрибуцијата на серверски custom грешки (`ln-validate:set-custom`) е на consumer/координаторски код, не на `ln-form`.
