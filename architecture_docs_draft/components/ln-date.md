...
### Варијанта 3: Форматирање на Семантички Не-Input Текст Елементи (`<time>`, `<td>`)

`ln-date` може да се примени и директно на семантички не-input текст елементи како што се `<time datetime="...">` или `<td>`. Го парсира ISO датумот од `datetime`, `data-ln-value`, `data-ln-date`, или внатрешниот текст, и го форматира `textContent` автоматски според активниот локал:

```html
<!-- Семантички HTML5 time елемент со datetime атрибут -->
<time datetime="2026-07-25" data-ln-date="medium">2026-07-25</time>
<!-- Приказ во mk/de: 25 јул 2026 / 25.07.2026 | Приказ во en-US: Jul 25, 2026 -->

<!-- Long формат со експлицитен data-ln-value -->
<time data-ln-value="2026-07-25" data-ln-date="long">2026-07-25</time>
<!-- Приказ во mk: 25 јули 2026 | Приказ во en-US: July 25, 2026 -->

<!-- Сопствен шаблон на ќелија од табела -->
<td data-ln-value="2026-07-25" data-ln-date="dd.MM.yyyy">2026-07-25</td>
<!-- Приказ: 25.07.2026 -->
```

> [!NOTE]
> За зачувување на семантиката и пристапноста (a11y), за приказ на датуми секогаш користете `<time datetime="YYYY-MM-DD">` наместо генерички `<span>` елементи. При каква било промена на `lang` атрибутот во родителот, сите `data-ln-date` текст елементи автоматски се преформатираат во реално време.
...
| `data-ln-date-dict` | `<ul>` | `String` | - | Ја дефинира јазичната ознака за декларативниот локален речник (на пр. `"mk"`). |
| `data-ln-date-dict-key` | `<li>` | `String` | - | Атрибут преку кој се дефинира клучот за превод на деновите и месеците од внатрешниот речник (на пр. `"months-long"`, `"months-short"`) за `buildDict`. |
| `data-ln-fill-as` | `<input>` | `String` | - | Се користи за мапирање на изборот во скриениот input за компатибилност со формите (се пренесува на скриеното поле). |
| `data-ln-date-label` | `<input>` | `String` | `"Open date picker"` | Овозможува развивачот да постави сопствен `aria-label` за копчето што го отвара календарчето (пристапност). |
...
| :--- | :--- | :--- | :--- |
| `ln-date:change` | `<input>` | `{ value, formatted, date }` | Се диспачира при секоја промена на датумот (преку избирач, рачен внес или програмски). |
| `ln-date:destroyed` | `<input>` | `{ target }` | Се емитува кога Date-picker-от ќе се деструктуира и ќе се врати во нативна состојба. |
