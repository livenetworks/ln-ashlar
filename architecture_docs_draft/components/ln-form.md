...
- **Ортогоналност (Што компонентата НЕ прави):**
  - **Submit е целосно нативен.** `ln-form` воопшто не слуша `submit` — нема validation gate, нема `preventDefault()` од негова страна. Гејтот (валидација при поднесување, без method услов — секој метод, GET вклучително) е исклучиво на [`ln-validate`](./ln-validate.md): штом формата содржи барем едно `data-ln-validate` поле, `novalidate` се инјектира автоматски и submit-от се блокира само при невалидни полиња. Валидна форма продолжува нативно; серијализацијата и create/update толкувањето се на [`ln-data-coordinator`](./ln-data-coordinator.md) единствено кога формата носи `data-ln-form-scope` — инаку продолжува кон native submit или [`ln-ajax`](../../js/ln-ajax/README.md).
