...
### Варијанта 3: Препорачана дво-елементна употреба (Two-Element Mode)

Ова е препорачаниот начин на употреба за копчиња што содржат покомплексна DOM структура (на пр. текст + икона или динамички бројачи на селектирани ставки). 

Се дефинираат два внатрешни спанови/елементи: еден за почетна состојба (`data-ln-confirm-idle`) и еден за состојба на потврда (`data-ln-confirm-active`). Компонентата само ги прикажува/крие овие елементи преку тоглирање на атрибутот `hidden`, спречувајќи ги несаканите бришења и рекреирања на DOM елементите.
...
| :--- | :--- | :--- | :--- |
| `data-ln-confirm` | `<button>`, `<a>` | `String` | Означувач за заштитник на акција. Вредноста е пораката за потврда (legacy режим). Ако се користи Two-Element режим, оваа вредност се остава празна. |
...
| `data-tooltip-text` | `<button>`, `<a>` | `String` (авто) | Управувана состојба од JS. Го прикажува натписот во tooltip балончето при легаси иконски режим. |
| `data-ln-confirm-idle` | Било кој | `Element` | Детски елемент на копчето кој ја дефинира почетната (Idle) состојба во Two-Element режим. Се прикажува кога нема активна потврда. |
| `data-ln-confirm-active` | Било кој | `Element` | Детски елемент на копчето кој ја дефинира состојбата на потврда во Two-Element режим. Стандардно се крие преку `hidden` или CSS. |
...
> [!CAUTION]
> **4. Лимит на одговорност: Само поединечни акции (Single-Element Actions)**
> `ln-confirm` е наменет исклучиво за поединечни, ниско-влијателни акциски копчиња (на пр. бришење ред во табела, архивирање на еден запис). Користење на оваа компонента за масовни деструктивни акции (Bulk Actions од типот на „Избриши ги селектираните објекти“) е строго забрането. За такви сценарија секогаш мора да се користи посебен потврден модал (`ln-modal`), каде ќе се наведат сите засегнати ресурси и ќе се овозможат јасни, независни копчиња за потврда и откажување.
