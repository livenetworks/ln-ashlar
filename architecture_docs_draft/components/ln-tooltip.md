...
### ARIA Спецификација
* **`aria-describedby`**: Автоматски го поврзува тригерот со уникатниот ID на портал балончето за време на приказот.
* **Управување со тастатура (`Tab` & `ESC`)**: Приказот реагира на фокус во Capture фаза (`focus`/`blur`), а притискање на `Escape` веднаш го затвора активниот tooltip.
