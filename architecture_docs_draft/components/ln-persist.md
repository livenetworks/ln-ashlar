...
>
> **3. Очекување на меѓу-странично (глобално) споделување на состојбата:**
> Бидејќи генерираниот клуч секогаш ја вклучува моменталната URL патека (`{pathname}`), сите зачувани состојби се строго изолирани по страница (path-scoped). Дури и ако користите експлицитен клуч во атрибутот (на пр. `data-ln-persist="global-status-filter"`), вредноста нема да се сподели помеѓу `/admin/users` и `/reports/users`. Модулот по дизајн не поддржува глобално перзистирање низ различни страници за да спречи несакани мешања на состојби.
