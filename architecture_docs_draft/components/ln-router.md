...
*   **Чистење на активни компоненти (Teardown Pipeline):** При секое напуштање на рута, рутерот рекурзивно ги пронаоѓа сите внатрешни активни компоненти во целниот регион и ги уништува (`.destroy()`), спречувајќи истекување на меморија (memory leaks). Исто така, ги чисти телепортираните popover елементи во `<body>` чии тригери биле дел од заменетата содржина.
...
*   **Заштита на Хаш промени (Hash Popstate Guard):** Спречува релоадирање и преиспитување на рутите доколку настанатиот `popstate` претставува промена само во хашот (hashchange) на URL-то, дозволувајќи им на хаш-компонентите (како `ln-modal`) да реагираат независно.
...
Транзициите се олеснети со автоматската интеграција на View Transitions API. CSS Engine-от може да ја дефинира формата на анимација:
...
*   **Честа грешка 3 (Хаш-конфликт):** Неправилно ракување со хашот при навигација назад. Доколку апликацијата користи `window.location.hash` за дијалози, `ln-router` правилно ќе ги игнорира овие промени. Но, ако самите линкови содржат погрешен формат на хаш, рутерот може погрешно да ги обработи.

---

## 6. Дијаграм на Текот и Животен Циклус (Транзиција со View Transition)
...
sequenceDiagram
    ...
    Router->>Outlet: Run _teardownOutlet() (Call destroy() on children)
...
*   **`ln-nav`**: Прецизно го следи `pushState` на `ln-router` за да ги означи соодветните линкови како активни.
*   **`ln-modal`**: Нејзиниот систем за отворање преку хаш работи во кохезија со рутерот без меѓусебни конфликти.
*   **`ln-popover`**: Нејзините отворени телепортирани инстанци во `body` се автоматски зачистени при деструкција на содржината на рутата.
