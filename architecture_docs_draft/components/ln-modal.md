...
#### Варијанта 4: Универзален НЗС протокол за зачувување на форми (`ln-form:success` и нативен submit)

За сите типови на форми (AJAX, `ln-data-store`, `ln-api-connector` или обичен нативен submit):

1. **AJAX и Local-First Зачувувања:** По успешно запишување, слојот на формата диспачира `ln-form:success` (или `ln-ajax:success`). `ln-modal` го пресретнува овој настан и автоматски поставува `data-ln-modal="close"`, со што го затвора модалот и го брише хешот од URL-то. Доколку бекендот врати грешка (HTTP 422), се диспачира `ln-form:error` и модалот останува отворен со прикажани инлајн грешки.
2. **Обичен Нативен Submit (без AJAX):** При клик на `Save`, `ln-modal` запишува привремен флаг во `sessionStorage` (`ln-modal-pending:<modalId> = "true"`), а `ln-autosave` ги зачувува внесените полиња во `localStorage`. По рефрешот:
   - Доколку бекендот врати **200 OK (Успех)**: `ln-modal` го детектира флагот без грешки, го чисти флагот, го чисти драфтот од `ln-autosave`, го брише хешот и го затвора модалот.
   - Доколку бекендот врати **Грешка**: `ln-modal` го детектира флагот со грешка, го ресетира флагот, а модалот останува отворен при што `ln-autosave` ги враќа внесените податоци.
   - При **Директен линк** (`#user-modal:42`): Во `sessionStorage` нема флаг, па модалот нативно се отвара и останува отворен за работа.
...
sequenceDiagram
    ...
    else Формата е поднесена со успех (ln-form:success / ln-ajax:success)
        Modal->>Modal: Настанот меури од формата до <dialog> контејнерот
        Modal->>Modal: Сетува атрибут data-ln-modal="close"
    else Обичен нативен submit (без AJAX) со превчитување
        Modal->>Browser: Нативно поднесување (POST) + запис во sessionStorage и ln-autosave
        Browser->>Modal: Страницата се превчитува (HTTP 200 OK)
        Modal->>Modal: Детектира pending флаг + 200 OK -> го чисти флагот, драфтот и хешот, и сетува data-ln-modal="close"
    end
